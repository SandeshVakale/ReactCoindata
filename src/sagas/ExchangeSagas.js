...
export function * getExchange (api, action) {
  const {uuid, referenceCurrencyUuid} = action
  // get current data from Store
  // const currentData = yield select(ExchangeSelectors.getData)
  // make the call to the api
  const response = yield call(api.getExchange, {uuid, referenceCurrencyUuid})
  ...
  if (response.ok) {
    ...
    yield put(ExchangeActions.exchangeFailure(response))
  }
}
